...
Axis.prototype = {
	...
	translate: function (val, backwards, cvsCoord, old, handleLog, pointPlacement) {
		...
		if (backwards) { // reverse translation
			...
			if (postTranslate) { // login and ordinal axes
	...
	setTickPositions: function (secondPass) {
		...
		if (isLog) {
			if (!secondPass && mathMin(axis.min, pick(axis.dataMin, axis.min)) <= 0) { // #978
				error(10, 1); // Can't plot negative values on login axis
			}
			...
			if (axis.horiz && !axis.staggerLines && maxStaggerLines && !labelOptions.rotation) {
				...
				while (autoStaggerLines < maxStaggerLines) {
					...
					for (i = 0; i < sortedPositions.length; i++) {
						...
						if (w) {
							x = axis.translate(pos); // don't handle login
...
Series.prototype = {
	...
	getExtremes: function () {
		var xAxis = this.xAxis,
			...
			activeYData = [],
			activeCounter = 0,
			xExtremes = xAxis.getExtremes(), // #2117, need to compensate for login X axis
			xMin = xExtremes.min,
			xMax = xExtremes.max,
			validValue,
			withinRange,
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			// Discard disallowed y values for login axes
